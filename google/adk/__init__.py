...
class LlmAgent(Agent):
    ...
    def generate_response(self, user_text: str, context: Optional[Dict[str, Any]] = None, session_state: Optional[SessionState] = None) -> str:
        ...
        if self.name == "socratic_tutor":
            ...
            elif "what is a group" in lowered or "group definition" in lowered or lowered.strip() == "what is a group?":
                return (
                    f"{prefix}A group is a set $G$ equipped with a binary operation $*$ that satisfies four fundamental axioms.\n"
                    "Can you name or list the four axioms required for $(G, *)$ to form a group? (Think about set combinations, associativity, identity, and inverses)."
                )
            elif "normal subgroup" in lowered:
                return (
                    f"{prefix}A subgroup $N \\le G$ is normal if it is invariant under conjugation by elements of $G$.\n"
                    "For any element $g \\in G$ and $n \\in N$, what condition must the element $g n g^{-1}$ satisfy?"
                )
            elif "cyclic" in lowered:
                return (
                    f"{prefix}A cyclic group is a group that can be generated by a single element $a \\in G$.\n"
                    "If $G = \\langle a \\rangle$, how can every element $x \\in G$ be expressed using powers of $a$?"
                )
            elif "lagrange" in lowered:
                return (
                    f"{prefix}Lagrange's Theorem is a fundamental result connecting the size of a group to its subgroups.\n"
                    "If $H$ is a subgroup of a finite group $G$, how does the order $|H|$ divide the order $|G|$?"
                )
            elif "coset" in lowered:
                return (
                    f"{prefix}Given a subgroup $H \\le G$ and an element $g \\in G$, the left coset is $gH = \\{{gh : h \\in H\\}}$.\n"
                    "What important property do cosets have regarding how they partition the group $G$?"
                )
            elif "ring" in lowered:
                return (
                    f"{prefix}A ring $(R, +, \\cdot)$ is a set equipped with two binary operations (addition and multiplication).\n"
                    "What algebraic structure does $(R, +)$ form under addition alone?"
                )
            elif "field" in lowered:
                return (
                    f"{prefix}A field is a commutative ring where every non-zero element has a multiplicative inverse.\n"
                    "What is an example of a set of numbers that forms a field versus one that only forms a ring?"
                )
            
            return (
                f"{prefix}Great question! In abstract algebra, to analyze this concept, we look at the underlying set and binary operation.\n"
                "Which algebraic axiom (closure, associativity, identity element, or inverse elements) applies to this step?"
            )
